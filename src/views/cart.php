    ...
    <?php if (!empty($cart)): ?>
                ...
                <?php foreach ($cart as $item): ?>
                    <tr style="border-bottom: 1px solid #ddd;">
                        <td style="padding: 10px; text-align: center;">
                            <img 
                                src="<?= BASE_URL . 'assets' . htmlspecialchars($item['image']) ?>" 
                        ...
                        <form action="<?= BASE_URL ?>cart?action=remove" method="POST">
                                ...
                                <button 
                                    type="submit" 
                                    style="padding: 5px 10px; background-color: #03362e; color: white; border: none; border-radius: 5px;">
                                    Supprimer
                                </button>
                            </form>
                        </td>
        ...
        <form action="<?= BASE_URL ?>cart?action=applyPromo" method="POST" style="margin-bottom: 20px;">
                ...
                <input 
                    ...
                    style="padding: 5px; width: 200px; margin-left: 10px;">
                <button 
                    type="submit" 
                    style="padding: 5px 15px; background-color: #03362e; color: white; border: none; border-radius: 5px;">
                    Appliquer
                </button>
            </form>
            ...
            <form action="<?= BASE_URL ?>checkout" method="POST">
                <button 
                    type="submit" 
                    style="padding: 10px 20px; background-color: #03362e; color: white; border: none; border-radius: 5px; font-size: 16px;">
                    Valider le panier
                </button>
            </form>
        </div>
